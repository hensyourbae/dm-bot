...
bot.on("message", (message) => {
    ...
    if(command === "help") {
        if (message.author.id !== config.yourid){
            return message.channel.send({embed: {
                color: 1,
                author: {
                    name: "Hello I'm SM Bot",
                    value: "You can see all the commands || If you need further help with something [Instagram](https://instagram.com/iamyourhens)"
                  },
                fields: [{
    ...
    if(command === "ping") {
        if (message.author.id !== config.yourid){
            return message.channel.send({embed: {
                color: 1,
                description: `Latency is ${new Date().getTime() - message.createdTimestamp}ms!`
            }});
        } 
    }
